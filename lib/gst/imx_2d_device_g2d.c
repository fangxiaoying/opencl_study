...
imx_g2d_alloc_mem(Imx2DDevice *device, PhyMemBlock *memblk)
{
  ...
  pbuf = g2d_alloc (memblk->size, 1);
  if (!pbuf) {
    GST_ERROR("G2D allocate %lu bytes memory failed: %s",
              memblk->size, strerror(errno));
    return -1;
  }

  memblk->vaddr = (guchar*) pbuf->buf_vaddr;
  memblk->paddr = (guchar*)(long) pbuf->buf_paddr;
                             ...
                             PhyMemBlock *src_mem, guint offset, guint size)
{
  ...
  pbuf = g2d_alloc (dst_mem->size, 1);
  if (!pbuf) {
    GST_ERROR ("g2d_alloc failed.");
    return -1;
  }
  dst_mem->vaddr = (gchar*) pbuf->buf_vaddr;
  dst_mem->paddr = (gchar*)(long) pbuf->buf_paddr;
  ...
  src.buf_paddr = (int)(long)(src_mem->paddr + offset);
  src.buf_size = src_mem->size - offset;
  dst.buf_handle = NULL;
  dst.buf_vaddr = dst_mem->vaddr;
  dst.buf_paddr = (int)(long)(dst_mem->paddr);
  ...
  GST_DEBUG ("G2D copy from vaddr (%p), paddr (%p), size (%ld) to "
      "vaddr (%p), paddr (%p), size (%ld)",
                               ...
                               PhyMemBlock *from, PhyMemBlock *to)
{
  ...
  src.buf_paddr = (int)(long)(from->paddr);
  src.buf_size = from->size;
  dst.buf_handle = NULL;
  dst.buf_vaddr = (void *)(to->vaddr);
  dst.buf_paddr = (int)(long)(to->paddr);
...
static gint imx_g2d_set_src_plane(struct g2d_surface *g2d_src, gchar *paddr)
{
  switch(g2d_src->format) {
    case G2D_I420:
    case G2D_YV12:
      g2d_src->planes[0] = (gint)(long)(paddr);
      g2d_src->planes[1] = (gint)(long)(paddr + g2d_src->width * g2d_src->height);
                            ...
                            Imx2DFrame *dst, Imx2DFrame *src, gboolean alpha_en)
{
  ...
  if (src->fd[1] >= 0)
  {
    if (!src->mem->user_data) {
      g2d->src.base.planes[1] = (int)phy_addr_from_fd (src->fd[1]);
      src->mem->user_data = (void**)(long)g2d->src.base.planes[1];
    }
    else
      g2d->src.base.planes[1] = (int)(long)src->mem->user_data;
  }
  ...
  g2d->dst.base.planes[0] = (gint)(long)(dst->mem->paddr);
  if (g2d->dst.base.format == G2D_NV12)
    g2d->dst.base.planes[1] = (gint)(long)(dst->mem->paddr + g2d->dst.base.width * g2d->dst.base.height);
